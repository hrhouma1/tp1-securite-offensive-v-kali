...
# Laboratoire 41 — Distinguer chiffrement et exposition d’un stockage cloud

## Mission et contexte

Examiner trois réglages d’un stockage fictif et corriger les mauvaises conclusions de son responsable.
...
- Outils : Python 3.
- Mode : Configuration synthétique — aucun service cloud.
...
Le chiffrement au repos et le contrôle d’accès répondent à des problèmes différents. Un objet chiffré au repos peut être transmis à un lecteur autorisé par une configuration trop large. La journalisation ajoute de la visibilité mais ne bloque pas automatiquement l’accès.

<details>
<summary>Comprendre simplement</summary>
<p>Un coffre solide ouvert à tous par le gardien reste accessible ; noter les visiteurs n’est pas verrouiller la porte.</p>
</details>
...
```bash
python3 -m json.tool travail/stockage.json
cp travail/stockage.json travail/copies/lab41-stockage.json
python3 - <<'PY'
import json
from pathlib import Path
x = json.loads(Path("travail/stockage.json").read_text())
print("Public :", x["public"], "Chiffré :", x["chiffrement"], "Journalisé :", x["journalisation"])
PY
```

Les trois booléens décrivent un modèle simplifié, pas la configuration exhaustive d’un produit réel.

### 3. Interpréter

L’étudiant explique comment public=true et chiffrement=true peuvent coexister sans protéger la confidentialité.
...
Modifier la copie pour supprimer l’accès public et activer la journalisation ; proposer un test de lecteur autorisé et un test de lecteur non autorisé.

## Défi autonome

Décrire la différence entre contrôle préventif, détectif et correctif pour ce cas.

<details>
<summary>Indice — à ouvrir après un premier essai</summary>
<p>Ne pas conclure « sécurisé » à partir du seul indicateur de chiffrement.</p>
</details>
...
Conserver les pièces dans `travail/preuves/` avec le préfixe `lab41`. Ne pas joindre tout le dictionnaire rockyou.txt, une clé privée ou un mot de passe réel. Le [barème commun proposé](../00-Evaluation-et-progression.md) évalue la méthode, la preuve et le raisonnement.
...
Ne pas conclure « sécurisé » à partir du seul indicateur de chiffrement.
