...
# Laboratoire 48 — Auditer une configuration Wi-Fi sans carte radio

## Mission et contexte

Évaluer une configuration fictive et justifier ce que l’on peut et ne peut pas vérifier sans matériel radio ni capture autorisée.
...
- Outils : Python 3, documentation du scénario.
- Mode : Analyse simulée — aucune émission radio.
...
WPA2-Personal (Wi-Fi Protected Access 2 en mode personnel, protection Wi-Fi à secret partagé) utilise une phrase commune aux appareils autorisés. WPS (Wi-Fi Protected Setup, configuration simplifiée du Wi-Fi) est un mécanisme distinct. La qualité de la phrase ne se mesure pas au seul nom du protocole.

<details>
<summary>Comprendre simplement</summary>
<p>Une porte moderne reste vulnérable si tous les visiteurs connaissent une clé trop évidente ou un accès de secours mal protégé.</p>
</details>
...
```bash
python3 -m json.tool travail/wifi-scenario.json
cp travail/wifi-scenario.json travail/copies/lab48-wifi.json
python3 - <<'PY'
import json
from pathlib import Path
x = json.loads(Path("travail/wifi-scenario.json").read_text())
mots = set(Path("travail/mots.txt").read_text().splitlines())
print("Phrase présente dans la liste d'exercice :", x["phrase"] in mots)
print("Capture radio fournie :", x["capture_radio"])
PY
```

SSID (Service Set Identifier, nom du réseau sans fil) est ici une chaîne fictive. Aucune interface radio, désauthentification ou recherche de réseau voisin n’est utilisée.

### 3. Interpréter

La phrase de démonstration est reconnue comme courante dans la liste et la limite d’absence de capture est clairement inscrite.
...
Proposer une phrase unique non prévisible, la désactivation de WPS si inutile et une séparation des usages, à valider sur un équipement autorisé.

## Défi autonome

Lister le matériel, l’autorisation et les données qu’exigerait une véritable validation radio dans un autre atelier.

<details>
<summary>Indice — à ouvrir après un premier essai</summary>
<p>Ne pas annoncer une clé WPA retrouvée à partir d’une capture : aucune capture n’existe dans ce laboratoire.</p>
</details>
...
Conserver les pièces dans `travail/preuves/` avec le préfixe `lab48`. Ne pas joindre tout le dictionnaire rockyou.txt, une clé privée ou un mot de passe réel. Le [barème commun proposé](../00-Evaluation-et-progression.md) évalue la méthode, la preuve et le raisonnement.
...
Ne pas annoncer une clé WPA retrouvée à partir d’une capture : aucune capture n’existe dans ce laboratoire.
