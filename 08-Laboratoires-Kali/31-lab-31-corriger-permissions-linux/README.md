...
# Laboratoire 31 — Corriger un fichier modifiable par tout le monde

## Mission et contexte

Identifier puis réduire des droits excessifs sur une copie de rapport fictif, sans toucher à un dossier système.
...
- Outils : chmod, stat, umask.
- Mode : Fichiers d’exercice uniquement.
...
Les permissions Linux distinguent propriétaire, groupe et autres utilisateurs. Les droits de lecture, d’écriture et d’exécution n’ont pas la même signification sur un fichier et sur un dossier. Un test effectué par le propriétaire ne démontre pas à lui seul les possibilités des autres utilisateurs.

<details>
<summary>Comprendre simplement</summary>
<p>Le propriétaire, son équipe et les visiteurs n’ont pas nécessairement les mêmes clés.</p>
</details>
...
```bash
cp travail/prive/message.txt travail/copies/lab31-rapport.txt
chmod 666 travail/copies/lab31-rapport.txt
stat -c '%a %U %G %n' travail/copies/lab31-rapport.txt
chmod 640 travail/copies/lab31-rapport.txt
stat -c '%a %U %G %n' travail/copies/lab31-rapport.txt
namei -l travail/copies/lab31-rapport.txt
```

666 autorise lecture et écriture pour les trois catégories ; 640 conserve lecture/écriture au propriétaire, lecture au groupe et aucun droit aux autres. namei affiche aussi les composants du chemin.

### 3. Interpréter

Les modes avant/après sont prouvés et les permissions des dossiers parents sont examinées.
...
Expliquer si le groupe choisi correspond au rôle métier. Ne pas appliquer chmod récursivement à tout le cours.

## Défi autonome

Créer un second fichier sous umask 077 dans un sous-shell et comparer son mode sans changer le masque de toute la session.

<details>
<summary>Indice — à ouvrir après un premier essai</summary>
<p>Réaliser les exercices de permissions dans le système de fichiers Linux de Kali, pas directement dans /mnt/c sous Windows.</p>
</details>
...
Conserver les pièces dans `travail/preuves/` avec le préfixe `lab31`. Ne pas joindre tout le dictionnaire rockyou.txt, une clé privée ou un mot de passe réel. Le [barème commun proposé](../00-Evaluation-et-progression.md) évalue la méthode, la preuve et le raisonnement.
...
Réaliser les exercices de permissions dans le système de fichiers Linux de Kali, pas directement dans /mnt/c sous Windows.
