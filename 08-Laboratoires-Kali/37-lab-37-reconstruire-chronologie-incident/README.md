...
# Laboratoire 37 — Reconstituer une chronologie sans inventer l’attaquant

## Mission et contexte

Construire une séquence de faits et deux hypothèses compatibles avec les traces.
...
- Outils : Python 3, datetime.
- Mode : Enquête sur traces synthétiques.
...
Une chronologie ordonne des événements et leurs sources. Une connexion réussie après des échecs peut avoir plusieurs explications. L’identité de compte n’identifie pas forcément la personne derrière l’action ; les horloges et fuseaux demandent aussi une vérification.

<details>
<summary>Comprendre simplement</summary>
<p>Des empreintes de pas donnent un ordre de passage, pas automatiquement le nom de celui qui marchait.</p>
</details>
...
for x in sorted(lignes, key=lambda x: datetime.fromisoformat(x["heure"].replace("Z", "+00:00"))):
    print(x["heure"], x["compte"], x["action"], x["statut"])
PY
sha256sum travail/evenements.jsonl
```

UTC (Coordinated Universal Time, temps universel coordonné) est représenté par Z dans ces horodatages. Toutes les heures appartiennent au scénario synthétique.

### 3. Interpréter

Deux échecs, une réussite, un export puis une lecture sont ordonnés sans attribution personnelle non démontrée.
...
Ajouter une colonne « source » et une colonne « interprétation ». Décrire la donnée qui permettrait de départager deux hypothèses.

## Défi autonome

Introduire deux événements de même heure dans une copie et expliquer la limite de précision.

<details>
<summary>Indice — à ouvrir après un premier essai</summary>
<p>Ne pas appeler une suite d’échecs une attaque confirmée sans autre élément.</p>
</details>
...
Conserver les pièces dans `travail/preuves/` avec le préfixe `lab37`. Ne pas joindre tout le dictionnaire rockyou.txt, une clé privée ou un mot de passe réel. Le [barème commun proposé](../00-Evaluation-et-progression.md) évalue la méthode, la preuve et le raisonnement.
...
Ne pas appeler une suite d’échecs une attaque confirmée sans autre élément.
